						...
						<CardContent className="p-0">
							<div className="flex items-center justify-between p-3">
								<div className="flex items-center gap-4">
									...
									<div className="flex items-center gap-3">
										...
										<span className="[font-family:'Geist',Helvetica] font-medium text-zinc-900 text-sm leading-5 overflow-hidden text-ellipsis">
											{task.title}
										</span>
									</div>
								</div>
								...
								<div className="flex items-center gap-2">
									{task.status === "Uploaded" ? (
										<span
											className={`[font-family:'Geist',Helvetica] font-medium text-sm leading-5 ${task.statusColor}`}>
											{task.status}
										</span>
									) : task.status === "Upload" ? (
							...
							{task.hasSubtasks && (
								<div className="relative flex gap-3 px-8 pb-3">
									...
									<img
										className="w-3.5 h-[148px] mt-1 flex-shrink-0"
										alt="Group"
									...
									<div className="flex-1 space-y-2">
										{task.subtasks?.map((subtask, subtaskIndex) => (
											<div
												...
												<div className="font-medium text-zinc-900 text-sm leading-5">
													{subtask.title}
												</div>
												<div className="flex items-center gap-1.5">
													...
													<Badge
														className={`px-2 py-0.5 ${subtask.badge.color} rounded-full text-[10px] font-semibold leading-[15px]`}>
														{subtask.badge.text}
													</Badge>
												</div>
											</div>
					...
					<Card
						...
						<CardContent className="flex items-center gap-3 p-3">
							...
							<div className="flex-1">
								<div className="[font-family:'Geist',Helvetica] font-medium text-zinc-900 text-sm leading-5 line-through">
									{task.title}
								</div>
